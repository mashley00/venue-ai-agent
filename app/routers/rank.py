...
# ---------- existing JSON rank endpoint (kept for API use) ----------
@router.post("/run")
def run_rank(payload: dict):
    ...
    # Export to CSV/XLSX for API/automation consumers
...
# ---------- NEW: Browser-friendly preview (HTML table) ----------
TABLE_COLUMNS = [
    ("rank", "Rank"),
    ("name", "Venue"),
    ("category", "Category"),
...
def _to_html_table(ranked: List[dict]) -> str:
    # small, readable subset of columns
    rows = []
    for v in ranked:
...
<body>
<h1>Venue Preview</h1>
<p class="hint">This is a quick on-screen preview. Use <code>/rank/run</code> for JSON/CSV/XLSX exports.</p>
{inner}
</body></html>"""
...
def preview(payload: dict):
    """
    One-step preview: discovery -> details -> scoring -> HTML table
    Body accepts the SAME input you used for /discover previously, e.g.:
...
def preview_sample():
    """Zero-input demo using the bundled sample payload."""
    payload = {
      "cities": ["Greenville, NC"],
      "zips": ["27834"],
      "radius_miles": 6,
